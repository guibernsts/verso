<?php
require 'connection.php';
$connection = new Connection();

$delete = $connection->query("DELETE FROM user_colors");
	
$delete = $connection->query("DELETE FROM users");

$insert = $connection->query("INSERT INTO users(name, email)
    VALUES
        ('Alice Smith', 'alice.smith@example.com'),
        ('Bob Johnson', 'bob.johnson@example.com'),
        ('Charlie Davis', 'charlie.davis@example.com'),
        ('Dana Lee', 'dana.lee@example.com'),
        ('Evan Williams', 'evan.williams@example.com'),
        ('Fiona Brown', 'fiona.brown@example.com'),
        ('George Miller', 'george.miller@example.com'),
        ('Hannah Wilson', 'hannah.wilson@example.com'),
        ('Ian Clark', 'ian.clark@example.com'),
        ('Julia Moore', 'julia.moore@example.com');
        
		");

?>

